...
const makeVehicle = (mileageKm = 0) => Vehicle.restore({
  id: 'vehicle-1', customerId: 'customer-1', plate: 'ABC1234',
  brand: 'Toyota', model: 'Corolla', year: 2022, color: 'White',
  mileageKm, active: true, createdAt: new Date(), updatedAt: new Date(),
});

const makeCustomer = () => Customer.restore({
  id: 'customer-1', documentType: DocumentType.CPF, documentNumber: '11144477735',
  name: 'João', email: 'j@e.com', phone: '11999999999',
  active: true, createdAt: new Date(), updatedAt: new Date(),
});

describe('VehicleService', () => {
  let service: VehicleService;
  let vehicleRepo: { findById: jest.Mock; findByPlate: jest.Mock; findByCustomerId: jest.Mock; findAll: jest.Mock; save: jest.Mock; existsByPlate: jest.Mock; hasActiveOrders: jest.Mock };
  let customerRepo: { findById: jest.Mock };
  let logger: { log: jest.Mock; warn: jest.Mock; error: jest.Mock };

  beforeEach(() => {
    vehicleRepo = {
      findById: jest.fn(), findByPlate: jest.fn(), findByCustomerId: jest.fn(),
      findAll: jest.fn(), save: jest.fn().mockResolvedValue(undefined),
      existsByPlate: jest.fn(), hasActiveOrders: jest.fn(),
    };
    customerRepo = { findById: jest.fn() };
    logger = { log: jest.fn(), warn: jest.fn(), error: jest.fn() };
    service = new VehicleService(vehicleRepo as any, customerRepo as any, logger as any);
  });
  ...
  describe('create', () => {
    it('creates vehicle when customer exists and plate is new', async () => {
      ...
      const result = await service.create({ customerId: 'customer-1', plate: 'ABC-1234', brand: 'Toyota', model: 'Corolla', year: 2022, color: 'White' });
    ...
    it('throws CustomerNotFoundError when customer does not exist', async () => {
      ...
      await expect(service.create({ customerId: 'missing', plate: 'ABC-1234', brand: 'Toyota', model: 'Corolla', year: 2022, color: 'White' }))
    ...
    it('throws VehicleAlreadyExistsError when plate exists', async () => {
      ...
      await expect(service.create({ customerId: 'customer-1', plate: 'ABC-1234', brand: 'Toyota', model: 'Corolla', year: 2022, color: 'White' }))
  ...
  describe('update', () => {
    ...
    it('throws InvalidMileageError when new mileage is less than current', async () => {
      ...
      await expect(service.update('vehicle-1', { mileageKm: 3000 })).rejects.toThrow(InvalidMileageError);
    });
  });
  ...
  describe('deactivate', () => {
    ...
    it('throws VehicleHasActiveOrdersError when active orders exist', async () => {
      ...
      await expect(service.deactivate('vehicle-1')).rejects.toThrow(VehicleHasActiveOrdersError);
    });

    it('throws VehicleNotFoundError when not found', async () => {
      vehicleRepo.findById.mockResolvedValue(null);
      await expect(service.deactivate('missing')).rejects.toThrow(VehicleNotFoundError);
    });
  });
});
