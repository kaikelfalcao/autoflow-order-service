  ...
  async onModuleInit(): Promise<void> {
    ...
    const channel = connection.createChannel({
      setup: async (currentChannel: ConfirmChannel) => {
        ...
        await currentChannel.assertQueue('order.payment.confirmed', { durable: true });
        ...
        await currentChannel.assertQueue('order.payment.failed', { durable: true });
        await currentChannel.bindQueue('order.payment.failed', 'payment.events', 'payment.failed');

        await currentChannel.assertQueue('order.payment.refunded', { durable: true });
        await currentChannel.bindQueue(
        ...
        await currentChannel.consume('order.payment.confirmed', async (message: ConsumeMessage | null) => {
          if (!message) return;
        ...
        await currentChannel.consume('order.payment.failed', async (message: ConsumeMessage | null) => {
          if (!message) return;
          const payload = JSON.parse(message.content.toString()) as PaymentEventPayload;
        ...
        await currentChannel.consume('order.payment.refunded', async (message: ConsumeMessage | null) => {
          if (!message) return;
          const payload = JSON.parse(message.content.toString()) as PaymentEventPayload;
