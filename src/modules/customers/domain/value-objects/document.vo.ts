...
export class Document {
  ...
  private static validateCpf(cpf: string): void {
    if (cpf.length !== 11)
      throw new InvalidDocumentError("CPF must have 11 digits");
    if (/^(\d)\1+$/.test(cpf))
      throw new InvalidDocumentError("CPF with all equal digits is invalid");
    ...
    if (remainder !== parseInt(cpf[9]))
      throw new InvalidDocumentError("Invalid CPF check digit");
  ...
  private static validateCnpj(cnpj: string): void {
    if (cnpj.length !== 14)
      throw new InvalidDocumentError("CNPJ must have 14 digits");
    if (/^(\d)\1+$/.test(cnpj))
      throw new InvalidDocumentError("CNPJ with all equal digits is invalid");
    ...
    if (d1 !== parseInt(cnpj[12]))
      throw new InvalidDocumentError("Invalid CNPJ check digit");

    const d2 = calc(cnpj, [6, 5, 4, 3, 2, 9, 8, 7, 6, 5, 4, 3, 2]);
    if (d2 !== parseInt(cnpj[13]))
      throw new InvalidDocumentError("Invalid CNPJ check digit");
  }
}
